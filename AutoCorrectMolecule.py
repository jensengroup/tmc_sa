...
def ParseArgs():
    ...
    parser.add_argument("dictionary", type=str, help="Path to chemical dictionary.")
    ...
    parser.add_argument(
        "-s", "--max_tree_size", type=int, default=1000, help="Maximum tree size."
    )
    parser.add_argument(
        "-d", "--max_tree_depth", type=int, default=10, help="Maximum tree depth."
    )
    parser.add_argument(
        "-n",
        "--n_solutions",
...
def Main():
    ...
    print(
        f"Search terminated after {result.n_expansions} expansions ({(end_time - start_time)/1e6:.0f} ms)"
    )
    for discovery in result.top_discoveries:
        print(
            (
                f"Molecule: {Chem.MolToSmiles(discovery.molecule)}, "
