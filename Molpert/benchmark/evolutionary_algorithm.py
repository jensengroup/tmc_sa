...
ReproductionType = Callable[
    [PopulationType, ScoresType, int], tuple[PopulationType, ScoresType]
]
...
class Reproduction:
    ...
    def AcceptChild(self, population: PopulationType, child: T) -> bool:
...
class EvolutionaryAlgorithm:
    ...
    def __call__(
        ...
        scores = np.fromiter((objective_function(i) for i in population), np.single)
        for generation in range(n_generations):
            population, scores = self.reproduction(
                population, scores, objective_function, self.n_children
            )
...
def FitnessProportionateSelection(
    ...
    return np.random.choice(
        biased_scores.size, size=n, replace=replace, p=sampling_probabilities
    )


def TruncationSelection(
    population: PopulationType, scores: ScoresType, n: int
) -> SelectedType:
...
def BreakTiesTruncationSelection(
    ...
    first_worst_idx = np.searchsorted(sorted_scores, worst_selected_score, side="left")
    last_worst_idx = np.searchsorted(sorted_scores, worst_selected_score, side="right")
    worst_selection = np.random.choice(
        sort_order[first_worst_idx:last_worst_idx],
        size=n_worst_selection,
        replace=False,
    )
...
class StuckOrScoreThresholdTermination:
    def __init__(self, max_n_generations_stuck: int, score_threshold: float):
        self.best_score = 0.0
    ...
    def __call__(self, population: PopulationType, scores: ScoresType) -> bool:
