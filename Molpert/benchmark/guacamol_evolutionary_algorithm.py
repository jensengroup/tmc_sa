...
from molecule_manipulation import Mimicry, MoleculePerturber, MoleculeSimilarity
...
def ParseArgs():
    parser = argparse.ArgumentParser(
        description="Evaluates the performance of \
        a Molpert-based evolutionary algorithm with the GuacaMol benchmarks.",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument("output", type=str, help="Path to output file or directory.")
    parser.add_argument(
        "-vc",
        "--valence_constraints",
        action="store_true",
    ...
    if os.path.isfile(args.output) and (len(args.benchmarks) > 1 or args.replicas > 1):
        raise parser.error(
...
def SizeBiasedTruncationSelection(
    ...
    size_threshold = 6.0
    sizes = np.clip(
        np.fromiter((m.GetNumAtoms() for m in population), np.single),
        0.0,
        size_threshold,
    )
    ...
    for benchmark_idx, replica_idx in benchmark_ids:
        ...
        population, scores, n_generations = evolution(
            ...
            termination_criterion=StuckOrScoreThresholdTermination(
                max_n_generations_stuck=1000, score_threshold=1.0
            ),
        )
