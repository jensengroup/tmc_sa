...
def RefreshConstraints(
    constraints: OptionalConstraints, molecule: Chem.Mol
) -> OptionalConstraints:
...
class MoleculePerturber:
    def __init__(
        self, constraints: OptionalConstraints = None, start_molecule_id: int = 0
    ):
    ...
    def RandomMolecule(self, seed: Chem.Mol, n_atoms: int, n_bonds: int) -> Chem.Mol:
...
class MoleculeEnumerator:
    def __init__(self, constraints: OptionalConstraints = None):
        self.perturber = mpt.MoleculePerturber()
...
def AtomKeyFingerprint(
    molecule: Chem.Mol, radius: int = 2
