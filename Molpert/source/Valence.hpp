...
int IntValenceDiff(double valence_diff) {
  ...
  // +3.0, which is correctly reflected by adding the ceiling of +1.5 (+2) to
  // +1.
...
double BondTypeValenceContribution(RDKit::Bond::BondType bond_type) {
  ...
  case RDKit::Bond::FIVEANDAHALF:
    return 5.5;
  default:
    throw std::runtime_error("Unknown valence contribution for bond type " +
                             bond_type);
  };
};
...
int AvailableValence(const RDKit::Atom *atom, bool include_hydrogens = true,
                     bool aromatic_is_single = false) {
  int max_allowed_valence = MaxAllowedValence(atom->getAtomicNum());
  if (max_allowed_valence < 0) {
    return std::numeric_limits<int>::max();
  };
  return max_allowed_valence -
         ExplicitValence(atom, include_hydrogens, aromatic_is_single);
};
...
MinNumHydrogensForValidValence(const RDKit::Atom *atom) {
  static const RDKit::PeriodicTable *periodic_table =
      RDKit::PeriodicTable::getTable();
  const std::vector<int> &allowed_valences =
