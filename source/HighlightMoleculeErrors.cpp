...
MolecularKeyErrors(const std::vector<AtomKey> &atom_keys,
...
std::pair<std::vector<unsigned>, std::vector<unsigned>> MolecularErroneousness(
  ...
  std::size_t n_atoms = atom_key_errors.size(),
              n_bonds = bond_key_errors.size();
  std::vector<unsigned> atom_erroneousness(n_atoms);
  std::vector<unsigned> bond_erroneousness(n_bonds);
  for (std::size_t atom_idx = 0; atom_idx < n_atoms; ++atom_idx) {
    atom_erroneousness[atom_idx] =
        static_cast<std::underlying_type_t<AtomKey::Error>>(
            atom_key_errors[atom_idx]);
  };
  for (std::size_t bond_idx = 0; bond_idx < n_bonds; ++bond_idx) {
    bond_erroneousness[bond_idx] =
        static_cast<std::underlying_type_t<BondKey::Error>>(
            bond_key_errors[bond_idx]);
  };
...
int main(int argc, char *argv[]) {
  ...
  unsigned n_atom_key_errors = std::count_if(
      ...
      [](AtomKey::Error error) { return error != AtomKey::Error::NONE; });
  if (n_atom_key_errors > 0) {
    std::cout << "Molecule has " << n_atom_key_errors << " foreign atom keys"
              << std::endl;
    if (verbose) {
      for (std::size_t atom_idx = 0; atom_idx < n_atoms; ++atom_idx) {
  ...
  unsigned n_bond_key_errors = std::count_if(
      ...
      [](BondKey::Error error) { return error != BondKey::Error::NONE; });
  if (n_bond_key_errors > 0) {
    std::cout << "Molecule has " << n_bond_key_errors << " foreign bond keys"
              << std::endl;
    if (verbose) {
      for (std::size_t bond_idx = 0; bond_idx < n_bonds; ++bond_idx) {
  ...
  if (foreign_environments.empty()) {
    ...
    if (verbose) {
      for (const CircularAtomicEnvironment &environment :
